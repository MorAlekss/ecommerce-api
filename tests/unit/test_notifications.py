...
def test_send_email():
    ...
    with patch('src.notifications.email.httpx.AsyncClient') as mock_ac:
        mock_ac.return_value.__aenter__.return_value = mock_client
        result = asyncio.run(
            send_email("token123", "user@example.com", "Hello", "Test body")
        )
...
def test_send_bulk_email():
    ...
    with patch('src.notifications.email.httpx.AsyncClient') as mock_ac:
        mock_ac.return_value.__aenter__.return_value = mock_client
        result = asyncio.run(
            send_bulk_email("token123", ["a@example.com", "b@example.com"], "Subject", "Body")
        )
