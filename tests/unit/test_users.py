...
async def test_get_profile():
    with patch('src.users.profile.httpx.AsyncClient') as mock_client:
...
async def test_list_users():
    with patch('src.users.admin.httpx.AsyncClient') as mock_client:
        mock_instance = MagicMock()
        mock_client.return_value.__aenter__.return_value = mock_instance
        mock_response = MagicMock()
...
async def test_get_preferences():
    with patch('src.users.preferences.httpx.AsyncClient') as mock_client:
        mock_instance = MagicMock()
        mock_client.return_value.__aenter__.return_value = mock_instance
        mock_response = MagicMock()
