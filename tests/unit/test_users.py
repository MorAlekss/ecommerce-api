...
# --- profile tests (still sync requests, not yet migrated) ---
...
# --- admin tests (migrated to async httpx) ---
...
async def test_list_users():
    ...
    with patch('src.users.admin.httpx.AsyncClient', return_value=mock_client):
        result = await list_users("admin_token")
        assert result["total"] == 2

# --- preferences tests (still sync requests, not yet migrated) ---
